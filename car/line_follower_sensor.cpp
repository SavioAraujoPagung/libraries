...
void Sensor::calibration_sensor(){
    uint16_t temp;
...
void Sensor::read_calibration_sensor_line(){
  uint16_t temp;
  for(int8_t j=FIM_SENSOR_LINHA; j >= INICIO_SENSOR_LINHA; j--){
...
static float Sensor::calculate_error(int16_t a, int16_t b) {
  if (a >= _FORA && b >= _FORA) { //os dois estão bastante na linha branca
    return a-b;
  }
  else if (a < _FORA && b >= _FORA) { //o A saiu da linha
    return (200 - b )* -1;
  }
  else if (a >= _FORA && b < _FORA) { // o B saiu da linha
    return (200 - a);
  }
  return 0;
}
...
static bool Sensor::in_line(int16_t s){
  if(s > _FORA) return true;
  else return false;
}

static bool Sensor::in_line(uint8_t e, uint8_t d){
  if(d > _FORA || e > _FORA) return true;
  else return false;
}
